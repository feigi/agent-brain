...
export function registerMemorySearch(server: McpServer, memoryService: MemoryService): void {
  server.registerTool(
    ...
    {
      description:
        'Search memories by semantic similarity. Returns ranked results with relevance scores. Example: memory_search({ project_id: "my-project", query: "database migration patterns" })',
      inputSchema: {
        project_id: z.string().describe("Project slug to search within"),
        query: z.string().describe("Natural language search query"),
        scope: z.enum(["project", "user"]).default("project").describe("Search scope"),
