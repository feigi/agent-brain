...
export function registerMemorySearch(server: McpServer, memoryService: MemoryService): void {
  server.registerTool(
    "memory_search",
    {
      ...
      inputSchema: {
        project_id: slugSchema.describe("Project slug to search within (e.g., 'my-project')"),
        query: z.string().describe("Natural language search query"),
        scope: z.enum(["project", "user", "both"]).catch("project").describe("Search scope: 'project' (default), 'user' (your memories), or 'both'"),
        user_id: slugSchema.describe("User identifier (e.g., 'alice'). Required for access control and user-scope filtering."),
