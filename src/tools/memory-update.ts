  ...
  server.registerTool(
    "memory_update",
    {
      ...
      inputSchema: {
        ...
        user_id: slugSchema.describe("Who is making this update (e.g., 'alice'). Required for access control."),
      },
    },
    async (params) => {
      return withErrorHandling(async () => {
        const { id, version, user_id, ...updates } = params;
        const result = await memoryService.update(id, version, updates, user_id);
