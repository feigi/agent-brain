...
function computeRelevance(
  ...
  // Composite score
  let relevance =
    SIMILARITY_WEIGHT * similarity + RECENCY_WEIGHT * recencyDecay;
...
if (options.scope === "project") {
  ...
  // scope === 'both' (D-10)
  if (!options.user_id)
    throw new Error("user_id is required for cross-scope search");
  conditions.push(
    or(
    ...
    "memory_session_start",
    {
      ...
      inputSchema: {
        ...
        context: z
          ...
          .optional()
          .describe(
            "What the agent is working on (used for relevance ranking)",
          ),
        limit: z
          .number()
          .int()
